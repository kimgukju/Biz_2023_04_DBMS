...
DDL 명령 : DBA 가 사용하는 명령들, 물리적 객체 생성, 제거, 변경을 수행하는 명령
    CREATE : 객체생성, 최초 프로젝트 시작할 때
    DROP : 객체제거, 최초 프로젝트 시작할 때 잘못 생성된 객체를 다시 만들고자 할 때
    ALTER : 객체변경, 사용중에 중요한 변경사항이 발생했을 때
        구조변경을 수행하는 명령
        구조변경은 매우 신중하게 수행해야 한다
*/

/*
주소 취미 Relaton Table 의 제 3 정규화
제2정규화가 되어 있는 tbl_addr_hobby TABLE 에는 
...
UPDATE 명령은 기본적으로 PK 를 기준으로 1개의 데이터만 변경되도록 하는것이
...
--      중복 없이 데이터를 모으기 
-- hb name 칼럼의 데이터를 같은 데이터끼리 그룹(묶어)지어 리스트를 만든다
...
/*
Table JOIN
정규화(2,3정규화) 된 Table 의 정보를 Select(조회)하면 단순히 1개의 Table 만으로는
...
-- EQ JOIN 은 연계하는 Table 간에 참조무결성 제약조건이 완벽 할 때는 
-- 아무런 문제가 없이 결과를 조회 할 수 있다
...
-- 2. 참조 무결성 제약 조건이 완벽하게 성립하는지 확인 하는 용도
