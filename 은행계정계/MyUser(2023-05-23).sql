...
-- 데이터가 없다 이런 경우는 Result(결과)가 Empty 인 상태이다
SELECT * FROM tbl_buyer WHERE buname = '홍길동';

/*
buname 칼럼을 기준으로 조건 설정하였다
buname 칼럼에 어떤 값을 조건으로 데이터를 조회 한 것
buname 칼럼은 데이터가 추가될 때 같은 이름의 데이터가 중보가여 저장될 수 있다
buname 칼럼을 기준으로 조회한 데이터가 비록 현재 1개만 보이더라도
...
현재 INSERT 명령을 실행하면서 buid에 해당하는 값을 지정(저장)하지 않았다
...
/*
TABLE을 create 하면서
buid 와 buname을 NOT NULL로 설정하였다.
데이터를 추가(INSERT) 할 때 최소한 buid 값과 buname 값을 필수로
필요하다 라는 선언
...
-- 매우 위험한 코드
-- Update, Delete 를 수행할때는 PK가 아닌 칼럼을 기준으로 하지 마라
...
1. buname 칼럼의 데이터가 성춘향 인 리스트를 조회
...
*/
 ...
 -- CRUD   update와 delete는 where 써줘야함 안쓰다간 재앙
 -- 데이터 추가(create)
...
/*
 현재 계좌정보를 조회했는데
 고객정보가 고객ID 뿐이어서 고객에 대한 이름, 전화번호 등을 알 수가 없다
 고객정보와 계좌정보를 연계하여 함께 볼 수 있다면 좋겠네.
 ...
 */
 ...
 java 는 DBMS가 응답하기를 기다리면서 무한정 기다리고 마치 프로젝트가 멈춘(Down)
 상태가 되어 버린다
