...
-- 잔액 순으로 정렬하여 보이기
-- 오름차순(ASCENDIG) 정렬 : 기본값 키워드 생략가능
SELECT * FROM tbl_acc
WHERE acBuid = '0001'
ORDER BY acBalance ASC;

-- 고객 ID 가 0001 인 고객의 계좌정보를
-- 잔액이 큰 것부터 순서대로 정렬하여 보이기
-- 내림차순(DESENDIG)
SELECT * FROM tbl_acc
WHERE acBuid = '0001'
...
-- acBuId에 0001 or 0003 의 반대인 경우
-- acBuId에 0001 도 아니고 0003 도 아닌 값의 조건 검색
SELECT * FROM tbl_acc
WHERE NOT acBuId IN ('0001','0003');

-- 위와 같은 결과의 코드
SELECT * FROM tbl_acc
WHERE acBuId != '0001' AND acBuId <> '0003'; -- '!=' 랑 '<>' 같다
