...
RDBMS(Relation DataBase Management System)에서 데이터 취급하기
    ...
    1. 물리적인 저장소 생성하기 = Oracle 에서는 TableSpace = CREATE TABLESPACE
...
-- 3. Update
-- Update를 수행할때는 정말 내가 변경하는 데이터가 정말 변경하고자 하는 데이터인가?
-- 홍길동 학생의 전화번호가 010-111-1111 에서 010-111-1234 로 변경되었다
-- 데이터를 수정하라

-- 일반적인 Update 명령은 다음과 같다. 문법적으로는 아무런 문제가 없으나
-- 결과적으로 심각한 문제를 안고있다
-- 홍길동 이라는 이름이 동명이인이 있을 경우, 원하지 않는 데이터가 변경 될 수 있다
...
-- 2. 조회된 데이터 중에서 변경하고자 하는데이터의 PK가 무엇인지 확인
-- 여기서는 230001이 확인되었다.
...
-- 이몽룡(전화번호가 010-333-1111) 학생이 자퇴를 하고 이민을 갔따
