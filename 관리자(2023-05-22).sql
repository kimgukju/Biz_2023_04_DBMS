...
--      데이터 파괴자명령, 매우 신중하게 주의하면서 사용해야 한다 
...
-- 2. DB 에 연결하여 실습(사용할) 일반 사용자 계정을 생성하기
...
-- 실습의 편리성을 위하여 SYSDBA 보다 한단계 낮은 일반 DBA 권한을 부여하자
-- 오라클에서는 SYSDBA(SYSTEM 등)의 User 권한은 매우 조심해서 신중하게 다루어야 한다
-- DB 사용의 편의성을 위하여 오라클에서는 DBA 권한을 일반 사용자에게 부여할 수 있도록
-- 하고 있다
