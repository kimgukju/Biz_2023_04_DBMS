...
-- SQL 을 사용한 간이 통계
-- 전체학생의 각 과목별 성적 총점 계산
-- 국어성적의 총점계산
-- SUM() : ANSI SQL 의 총 합계를 계산하는 통계 함수
-- AVG() : ANSI SQL 의 평균을 계산하는 통계 함수
...
-- 성적표와 과목정보가 완전 참조 관계 일때는 EQ JOIN 을
-- 사용할 수 있다
SELECT sc_stnum, sc_bcode, b_name, sc_score
...
-- 제 3정규화가 되어 있는 데이터를 PIVOT 보고서 형식으로 출력
-- IF(A,B,C) 조건이 A가 true면 B 아니면 C 
SELECT sc_stnum,
	SUM(IF(sc_bcode = 'B001',sc_score,0)) AS 국어,
    SUM(IF(sc_bcode = 'B002',sc_score,0)) AS 영어,
    SUM(IF(sc_bcode = 'B003',sc_score,0)) AS 수학,
...
공통
    ...
    UPDATE 명령
        ...
        이 전화번호를 010-222... 으로 바꿔라 (해당 PK 찾아서 바꾸기)
*/